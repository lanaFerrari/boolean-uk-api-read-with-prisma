...
function buildAnimalDatabase() {
  ...
  animalTypes.forEach(type => {
    ...
    const numberOfBreeds = getRandomInt(1, 3)

    for (let i = 0; i < numberOfBreeds; i++) {
      const breed = breedGenerator()

      const numberOfAnimals = getRandomInt(3, 5)
...
function buildBooksDatabase() {
  const numberOfFictionAuthors = getRandomInt(7, 10)
  ...
  const numberOfNonFictionAuthors = getRandomInt(7, 10)
